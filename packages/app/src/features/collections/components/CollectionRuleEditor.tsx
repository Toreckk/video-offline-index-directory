...
export function CollectionRuleEditor({ tags, durationBounds, value, onChange }: {
  ...
  return (
    <div>
      ...
      <RuleGroupEditor group={value.root} tags={tags} durationBounds={durationBounds} depth={0} isRoot onUpdate={updateNode} onRemove={removeNode} />
    </div>
  )
}

function RuleGroupEditor({ group, tags, durationBounds, depth, isRoot, onUpdate, onRemove }: {
    ...
    <section className={`border ${isRoot ? 'border-primary/30 bg-surface-dim/55 p-4' : 'border-white/10 bg-black/15 p-3'}`}>
      ...
      <div className="mt-3 space-y-2">
        {group.children.map((child) => child.kind === 'group'
          ? <RuleGroupEditor key={child.id} group={child} tags={tags} durationBounds={durationBounds} depth={depth + 1} onUpdate={onUpdate} onRemove={onRemove} />
          : <RuleRow key={child.id} node={child} tags={tags} durationBounds={durationBounds} onUpdate={onUpdate} onRemove={onRemove} />)}
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        <button type="button" onClick={() => addChild({ id: createCollectionRuleId(), kind: 'duration', range: durationBounds ? { mode: 'known', minimumSeconds: durationBounds.minimumSeconds, maximumSeconds: durationBounds.maximumSeconds } : { mode: 'unknown' } })} className="flex items-center gap-2 border border-white/10 px-3 py-2 text-xs font-black"><Clock3 size={14} />Add duration rule</button>
...
function RuleRow({ node, tags, durationBounds, onUpdate, onRemove }: {
