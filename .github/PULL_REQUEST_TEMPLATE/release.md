...
- [ ] Manual publication dispatch is the active gate; optional GitHub reviewer/protection settings are recorded accurately rather than assumed.
- [ ] Generated installers are absent from the Git diff.

### Merge effect

During assembly, `publish: false` prevents publication. Finalization sets it to true after the gates above. Merging that finalized proposal into `master` changes `release-manifest.json` and starts read-only validation and packaging at the merge SHA. The separate publisher requires a manual **Publish release** workflow dispatch on `master` with the reviewed build run ID and exact merge SHA; configured `release` environment reviewers add another approval layer. Publication creates the tag/release with installers, checksums and build provenance only after verification. It does not enable signing, auto-updates or web hosting.
...
- [ ] Publication workflow completed its independent public-download hash verification.
